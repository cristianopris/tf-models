...
class FasterRCNNMetaArch(model.DetectionModel):
  ...
  def _predict_second_stage(self, rpn_box_encodings,
    ...
    """
    #xxx stop gradient from propagating to RPN stage
    tf.stop_gradient(rpn_box_encodings)
    tf.stop_gradient(rpn_objectness_predictions_with_background)
    tf.stop_gradient(rpn_features_to_crop)
  ...
  def postprocess(self, prediction_dict, true_image_shapes):
    ...
    with tf.name_scope('FirstStagePostprocessor'):
      if self._number_of_stages == 1:
        norm_proposal_boxes, proposal_scores, num_proposals = self._postprocess_rpn(
            ...
            true_image_shapes)
        return {
            fields.DetectionResultFields.detection_boxes: norm_proposal_boxes,
...
#     print('num_detections:', num_detections)
    ...
    # [1, num_detections]
    avg_class_predictions = tf.reshape(tf.tile(tf.argmax(avg_class_predictions, axis=1), num_proposals), tf.shape(proposal_scores))
...
#                              )
  ...
  def _loss_box_classifier(self,
    ...
    with tf.name_scope('BoxClassifierLoss'):
      ...
      classification_loss = tf.Print(classification_loss, [tf.shape(classification_loss), classification_loss],
                                         '>>>> classification_loss: ')
  ...
  def _loss_box_classifier_orig(self,
                           refined_box_encodings,
                           class_predictions_with_background,
                           proposal_boxes,
                           num_proposals,

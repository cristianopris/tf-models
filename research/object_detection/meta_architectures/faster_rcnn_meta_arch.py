...
class FasterRCNNMetaArch(model.DetectionModel):
  ...
  def _predict_second_stage_my(self, rpn_box_encodings,
                            rpn_objectness_predictions_with_background,
                            rpn_features_to_crop,
                            anchors,
                            image_shape,
    ...
    with tf.name_scope('BoxClassifierLoss'):
      ...
      for i in range(len(groundtruth_classes_with_background_list)):
        groundtruth_classes_with_background_list[i] = tf.Print(groundtruth_classes_with_background_list[i],
                                                               ...
                                                               '>>>> groundtruth_classes_with_background_list[%s]: ' % i,
                                                               summarize=1000,
                                                               )
      ...
      for anchors, gt_boxes, gt_class_targets, gt_weights in zip(
        ...
        cls_targets = tf.reshape(tf.tile(gt_class_targets[0], [num_anchors]), [num_anchors , num_classes])
        cls_weights = tf.ones([num_anchors])
      ...
      second_stage_cls_loss = tf.reduce_mean(
          tf.boolean_mask(second_stage_cls_losses, paddings_indicator))
